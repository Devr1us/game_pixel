/* ============================================================
   SHADOW BLADE CHRONICLES — GAME ENGINE
...
class Player {
  ...
  update(map, objects, dt=1) {
    ...
    if (jumpKey) {
      if (this.onGround) {
        ...
        Particles.emit(this.x + this.w/2, this.y + this.h/2, 6, PAL.swordGlow, 0, 0, 3, 20);
      }
    }
    ...
    if (G.justPressed['ShiftLeft'] || G.justPressed['ShiftRight']) {
      if (this.dodgeCooldown <= 0) {
        ...
        Particles.emit(this.x + this.w/2, this.y + this.h/2, 8, PAL.swordGlow, 0, 0, 2, 15);
      }
    }
  ...
  get attackBox() {
    ...
    const ax = this.facing === 1 ? this.x + this.w : this.x - 28;
    return { x: ax, y: this.y + 4, w: 28, h: 24 };
  }
  ...
  draw() {
    ...
    // Sword
    if (this.attacking) {
      ctx.fillStyle = PAL.sword;
      const sx = f === 1 ? cx + this.w : cx - 24;
      ctx.fillRect(sx, cy + 8, 24, 5);
      ctx.fillStyle = PAL.swordGlow;
      ctx.fillRect(sx + (f===1?18:-4), cy + 6, 8, 9);
      // Attack particles
      if (this.attackTimer % 3 === 0) {
        Particles.emit(
          this.x + (f === 1 ? this.w + 12 : -12),
          this.y + 12, 2, PAL.swordGlow, f * 3, -1, 2, 12
        );
      }
    } else {
      // Held sword (small)
      ctx.fillStyle = PAL.sword;
      const hsx = f === 1 ? cx + 18 : cx - 4;
      ctx.fillRect(hsx, cy + 12, 12, 3);
    }
  }
}
